...
Runtime provides to instinct code:
  ...
  M5, Imu, Speaker, Synth, Widgets

`Synth` is the MIDI voice (a SAM2695 GM module). On real hardware it sends
MIDI bytes over a UART; here it's a print-only placeholder until that driver
lands (see _MidiStub below). The creature-sim simulator substitutes its own
capturing Synth, so the instinct's MIDI is recorded there regardless.
"""
...
# ── Synth (MIDI voice) ──────────────────────────────────────────────────────
# TODO: replace this print-only placeholder with the real SAM2695 driver —
# open a UART (standard MIDI is 31250 baud) and write the raw status/data bytes
# (note_on 0x90|ch, note_off 0x80|ch, program 0xC0|ch, control 0xB0|ch). The
# method signatures below are the contract the simulator's Synth already mirrors,
# so the instinct code is identical on-device and in the sim — only this class
# changes. Until the module is wired, on-device MIDI is just logged to the REPL.
class _MidiStub:
    def program(self, ch, program):
        print("Synth.program", ch, program)

    def note_on(self, ch, note, velocity=80):
        print("Synth.note_on", ch, note, velocity)

    def note_off(self, ch, note, velocity=0):
        print("Synth.note_off", ch, note)

    def note(self, ch, note, ms, velocity=80):
        print("Synth.note", ch, note, ms, velocity)

    def control_change(self, ch, control, value):
        print("Synth.control_change", ch, control, value)
