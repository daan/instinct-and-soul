...
(`now` = time.ticks_ms() / 1000).
    ...
    Calc.Pose(beta=0.08)   # Madgwick AHRS — accelerometer + gyroscope fusion
        p.update(ax, ay, az, gx, gy, gz, now) -> (wx, wy, wz): world-frame linear
        acceleration — the body's motion THROUGH SPACE, gravity removed and steady
        no matter how the sensor is twisted. Also p.up() -> gravity 'up' as a
        3-vector in the sensor frame (tilt/pose); p.quat() -> (w, x, y, z). Fuses
        the accelerometer (absolute tilt from gravity) with the gyroscope (smooth
        turning) into a real orientation, so you can read the body as VECTORS IN
        SPACE rather than raw axes. Tilt (pitch/roll) is solid; heading (yaw) can
        drift slowly (this body has no magnetometer).

The way to anticipate, not react: **clean** the signal -> **detect** its events
-> **estimate** their period -> **predict** the next one and act to MEET it,
correcting your estimate each time a real event lands. Acting on the prediction
arrives WITH the motion; thresholding the latest sample always arrives after.
    clean = Calc.OneEuro(); event = Calc.Onset()
    period = Calc.Periodicity(); predict = Calc.AlphaBeta()
    # each loop, with `feature` a signal you derive from the sensor:
    s = clean.update(feature, now)
    period.push(s, now)
    if event.step(s, now):
        predict.correct(now, period.period())
    if predict.due(now, lead_ms=20):
        ...                                # act the instant the next event is predicted
These are tools, not a plan — use only what earns its place.
