...
changes and program changes are not recorded.
...
(`now` = time.ticks_ms() / 1000).
    ...
    Calc.Periodicity(win_s=2.0, min_s=0.3, max_s=1.2)
        ...
        enough events — low cv is earned, never assumed. Feed it a smoothed
        feature; it recomputes only a few times a second.
