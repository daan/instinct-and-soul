...
(`now` = time.ticks_ms() / 1000).
    ...
    Calc.Periodicity(win_s=2.0, min_s=0.3, max_s=1.2)
        ...
        how predictable) the recurrence is; cv() reads 9.9 until it has observed
        enough events — low cv is earned, never assumed. observe() takes the
        moments of real detected events ONLY: feeding it a timer or a loop
        counter measures your own clock, not the body, and the perfect cv it
        returns is a lie you told yourself. Feed push() a smoothed feature; it
        recomputes only a few times a second.
    Calc.Flow(leak=2.0, min_speed=0.4)
        f.update(wx, wy, wz, now) -> (vx, vy, vz); f.reversal() -> None | (axis, sign)
        leaky integrator over Madgwick's world-frame acceleration -> the body's
        TRAVEL through space: a signed velocity vector (~m/s, world axes), and
        the moment that travel turns around — reversal() fires once, just after
        the dominant axis flips direction (a swing's turnaround, a stroke's
        landing). The leak bounds integration drift, so treat the magnitude as
        short-horizon speed, not an odometer: direction and the rhythm of
...
Also in scope: asyncio, time, struct, math, M5, Imu, Synth, Ear, Mem, Calc.
