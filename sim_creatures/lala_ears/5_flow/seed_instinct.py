async def run():
    ...
    # Warm-start the orientation across rewrites: a Madgwick reset mid-dance
    # cannot converge (the accelerometer never shows clean gravity), and a
    # wrong 'up' silences Flow's reversals for many seconds. My bearings are
    # state worth keeping, so they live in Mem like my report clock.
    pose = Calc.Madgwick(beta=0.1, q=Mem.latest("pose_q"))
    flow = Calc.Flow()             # travel through space + its turnarounds
    ...
    while True:
        ...
        ux, uy, uz = pose.up()
        vx, vy, vz = flow.update(wx, wy, wz, now)   # travel, world frame
        ...
        if now - win_start >= 4.0:
            ...
            Mem.push("pose_q", list(pose.quat()))   # keep my bearings for the next me
            win_start = now
            win_sum = 0.0
            win_peak = 0.0
            win_n = 0
