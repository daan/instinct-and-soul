async def run():
    ...
    win_start = 0.0          # start of the current observation window (s)
    win_sum = 0.0            # accumulated intensity over the window
    win_peak = 0.0
    win_n = 0
    last_report_t = -21.0    # so the first routine report comes early
    ...
    while True:
        ...
        if now - win_start >= 4.0:
            ...
            if moving_quiet or still_loud or overdue:
                ...
                send("{} last {:.0f}s: body mean={:.1f} peak={:.1f}; voiced lead={} other={} lead_gap={}".format(
                    tag, now - win_start, mean_i, win_peak, lead, other, ioi))
                last_report_t = now
            win_start = now
            win_sum = 0.0
            win_peak = 0.0
