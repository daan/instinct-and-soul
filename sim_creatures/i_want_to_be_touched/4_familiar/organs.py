...
creatures even when the world outside is identical), an Ear (the sense of
one's own voice, inherited from the lala_ears line), and a Together sense
(contingency: does sounding-while-alone actually summon contact, measured
against a matched-silence baseline — promoted to an organ after a 2_hunger
soul announced "tracking how quickly" a new call style drew the person back
and never managed to build the mechanics).
...
class _EarSynth:
    ...
    def note_on(self, ch, note, velocity=80):
        t = time.ticks_ms()
        _record([t, "on", ch, note, velocity])
        _together.note(t, _alone_now_s())
    ...
    def note(self, ch, note, ms, velocity=80):
        t = time.ticks_ms()
        _record([t, "note", ch, note, velocity, ms])
        _together.note(t, _alone_now_s())
...
class _HandlingState:
    ...
    def feed(self, a, g, now_s):
        ...
        _together.tick(_alone_now_s(), now_s * 1000.0)
...
# ── Together (contingency: does my voice reach them?) ───────────────────────

class _TogetherState:
    """Measures whether sounding-while-alone summons contact, against an
    honest baseline. Derives entirely from streams the body already owns —
    the Ear's record of what sounded (any voicing while alone IS an
    invitation, whatever the code calls it) and Handling's contact stream.
    Nothing to feed, nothing to fake: an unanswered call is recorded exactly
    like an answered one.

    An ATTEMPT = a burst of notes voiced while alone (>=3 s since contact),
    bursts separated by >3 s of silence. It is ANSWERED if contact arrives
    within 20 s of the burst's last note, else unanswered.
    The BASELINE: matched phantom attempts — moments while alone with no
    sound in the preceding 5 s — scored by the same 20 s rule. Comparing the
    two rates is the only honest answer to "do my calls work, or would they
    have come back anyway?"."""
    ...
    def __init__(self):
        self.attempts = []          # [t_last_note_ms, notes, answered_s or -1]
        self.open = None            # [t_first_ms, t_last_ms, notes]
        self.phantoms = [0, 0]      # [answered, total] closed phantoms
        self.open_phantom = None    # t_ms
        self.last_phantom_t = -1e12
        self.last_note_t = -1e12

    def note(self, now_ms, alone_s):
        self.last_note_t = now_ms
        if alone_s < self.ALONE_MIN_S:
            return
        if self.open is not None and now_ms - self.open[1] <= self.BURST_GAP_S * 1000:
            self.open[1] = now_ms
            self.open[2] += 1
        else:
            self._close_attempt(answered_s=None, now_ms=now_ms)
            self.open = [now_ms, now_ms, 1]

    def _close_attempt(self, answered_s, now_ms):
    ...
    def tick(self, alone_s, now_ms):
        in_contact = alone_s < 0.5
        if in_contact:
            if self.open is not None:
                dt = (now_ms - self.open[1]) / 1000.0
                self._close_attempt(dt if dt <= self.ANSWER_S else None, now_ms)
            if self.open_phantom is not None:
                if (now_ms - self.open_phantom) / 1000.0 <= self.ANSWER_S:
                    self.phantoms[0] += 1
                self.phantoms[1] += 1
                self.open_phantom = None
            return
        # alone: expire overdue attempt/phantom windows
        if self.open is not None and now_ms - self.open[1] > self.ANSWER_S * 1000:
            self._close_attempt(None, now_ms)
        if (self.open_phantom is not None
                and now_ms - self.open_phantom > self.ANSWER_S * 1000):
            self.phantoms[1] += 1
            self.open_phantom = None
        # seed a phantom: alone, quiet lately, none already open
        if (alone_s >= self.ALONE_MIN_S and self.open is None
                and self.open_phantom is None
                and now_ms - self.last_note_t > 5000
                and now_ms - self.last_phantom_t > self.PHANTOM_EVERY_S * 1000):
            self.open_phantom = now_ms
            self.last_phantom_t = now_ms


_together = _TogetherState()


class _Together:
    """The score of the only social question this body can ask: when I sing
    into an empty room, do they come? Counts, never conclusions — with few
    attempts the numbers are small and mean little."""

    def attempts(self, n=None):
        """The last n invitation-attempts (all if None), oldest first:
        [t_ms_of_last_note, notes_in_burst, answered_s] — answered_s is
        seconds from the burst's last note to contact, or -1 if nothing came
        within 20 s. Any voicing while alone counts as an attempt, whatever
        your code called it."""
        a = _together.attempts
        if n is None or n >= len(a):
            return list(a)
        return a[-int(n):]

    def answered(self):
        """(answered, total) over all attempts this session."""
        a = _together.attempts
        return (sum(1 for x in a if x[2] >= 0), len(a))

    def baseline(self):
        """(answered, total) for matched SILENT stretches while alone — how
        often they came back within 20 s of you doing nothing. If your
        answered() rate doesn't beat this, your calls are decoration."""
        return (_together.phantoms[0], _together.phantoms[1])
...
class _FamiliarState:
    ...
    R = 0.85           # assignment radius in normalized feature space
    ...
    def _vec(self, ep):
        ...
        # dur is down-weighted: episode segmentation adds noise to it, and a
        # gesture's identity lives more in its manner (direction, vigor,
        # restlessness) than in how long one instance happened to last.
        return [math.log(dur_s) / 2.0,
                math.log(max(5.0, ep[2])) / 1.5,
                ep[3] / 0.6,
                min(8.0, ep[4] / dur_s) / 3.0,
                ep[6] / 0.32]
