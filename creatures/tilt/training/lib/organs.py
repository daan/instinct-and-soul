...
# ── instinct tempo (TRAINING condition only) ────────────────────────────────
# The seed's clock, hoisted from the instinct to module level so the tuner's
# `set` reaches it live (set FROZEN_AFTER_S 1200). This deliberately bends
# the measure/meaning division for the bench: condition_1 keeps these inside
# seed_instinct.py where they belong. The seed reads them through the Tempo
# scope name every loop tick, so a `set` lands within a second.
# Defaults are BENCH tempo; worn tempo in parens (condition_1's values).
FROZEN_AFTER_S = 120.0   # a statue after this long of stillness (worn: 1200)
CALL_EVERY_S = 60.0      # restless call spacing while frozen persists (300)
OUTCOME_S = 45.0         # nothing by then = ignored; keep < CALL_EVERY_S so
                         # each act closes before the next call (worn: 240)
HUSH_WINDOW_S = 5.0      # a tap this soon after a call = hushed (worn: 5)
HUSH_GRACE_S = 300.0     # hushed: quiet this long, escalation resets (1800)
...
def attach(scope):
    ...
    scope["Tap"] = _Tap()
    # TRAINING: the module itself, so Tempo.FROZEN_AFTER_S is a live read
    # of the (tuner-patchable) globals above — never a snapshot
    scope["Tempo"] = sys.modules[__name__]
