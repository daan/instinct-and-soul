async def run():
    # TILT baseline, v2 — coherent with character.md: "restless when they're
    # FROZEN, content when they MOVE. Easily hushed, no grudges."
    #
    ...
    #
    # Bench tempo: FROZEN_AFTER_S=120, CALL_EVERY_S=60, HUSH_GRACE_S=300
    FROZEN_AFTER_S = 20 * 60   # a statue after this long of stillness
    CALL_EVERY_S = 300.0       # restless call spacing while frozen persists
    OUTCOME_S = 240.0          # nothing by then = ignored (< CALL_EVERY_S:
                               # each act closes before the next call)
    HUSH_WINDOW_S = 5.0        # a tap this soon after a call = hushed
    HUSH_GRACE_S = 30 * 60     # hushed: quiet this long, escalation resets
    ...
    while True:
        ...
        if pending is not None and now - pending[1] > OUTCOME_S:
            send("ACT#{} OUTCOME ignored (nothing in {:.0f}min)".format(
                pending[0], OUTCOME_S / 60))
            pending = None

        # ── restlessness: frozen too long -> invite movement ──
        if (still > FROZEN_AFTER_S and now > hushed_until
                and pending is None and now - last_call > CALL_EVERY_S):
