"""
main.py — M5StickS3 "midi_dancer" runtime (IMU + Grove SAM2695 GM synth).

Same spine as the base sticks3 runtime — boots M5 hardware, brings up WiFi,
opens a WebSocket to the spine, and runs an asyncio loop with heartbeat +
instinct-hotswap tasks — but the voice is an M5Stack Unit-Synth (SAM2695)
plugged into the Grove port, driven as MIDI-over-UART, instead of the built-in
speaker. It also injects the `Mem` and `Calc` toolkits so an instinct ported
from the simulator behaves the same on the board.

Runtime provides to instinct code:
  send(msg), asyncio, time, struct, math, M5, Imu, Synth, Mem, Calc

Device-side libraries flashed to /lib (creatures/midi_dancer/lib/*.py):
  synth.py         -> Sam2695Synth  (the `Synth` voice)
...
# creature drivers land there (creatures/midi_dancer/lib/*.py -> /lib/ via
...
try:
    Synth = Sam2695Synth()
    print("synth: SAM2695 ready on Grove UART")
