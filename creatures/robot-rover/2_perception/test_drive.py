"""
test_drive.py — bench checks for robot-rover 2_perception, no rover required.

    python creatures/robot-rover/2_perception/test_drive.py

Stage 1's three checks, plus this stage's fourth — the mm bracket:
  ...
  4. the mm bracket: a bout with a fresh echo at both ends carries mm0/mm1/mm,
     and EVERY weaker case — no ranger, a ranger that raises, no echo (0), a
     stale reading at either end — comes back None rather than a guess.
...
# ── 4. the mm bracket ──────────────────────────────────────────────────────
# The ranger the runtime installs returns (mm, age_ms) or None. The bout
# samples it at open and close; anything less than a fresh echo at BOTH ends
# must come back None — a half-bracket is a guess wearing a measurement.
print("mm bracket:")
R = [None]
drive.set_ranger(lambda: R[0])


def bracket_bout(mm_open, mm_close, kind="forward"):
    d = drive._DriveState()
    R[0] = mm_open
    d.note_command(kind, 40)
    for i in range(40):
        CLOCK[0] += 50
        d.feed((0.12 if i % 2 else -0.12, 0.0, -1.0), (0.0, 0.0, -4.0),
               CLOCK[0] / 1000.0)
    R[0] = mm_close
    d.note_command("stop", 0)
    return d.last_bout
...
b = bracket_bout((612, 50), (447, 40))
print("  fresh both ends:   ", {k: b[k] for k in ("mm0", "mm1", "mm")})
assert (b["mm0"], b["mm1"], b["mm"]) == (612, 447, 165), b

b = bracket_bout((612, 50), (720, 40), kind="backward")
assert b["mm"] == -108, b
print("  backward:           mm comes back negative (%d)" % b["mm"])

for label, mm_open, mm_close in (
        ("no echo at open",    (0, 10),    (500, 10)),
        ("no echo at close",   (612, 50),  (0, 10)),
        ("stale at close",     (612, 50),  (500, 900)),
        ("stale at open",      (612, 900), (500, 10)),
        ("no sensor",          None,       None),
):
    b = bracket_bout(mm_open, mm_close)
    assert b["mm"] is None, (label, b)
    print("  %-19s mm=None, correctly" % (label + ":"))

# a ranger that raises must cost nothing
drive.set_ranger(lambda: 1 // 0)
b = bracket_bout(None, None)
assert b is not None and b["mm"] is None, b
print("  raising ranger:     bout still scored, mm=None")

# and with no ranger installed at all (stage-1 behaviour), keys still present
drive._ranger[0] = None
b = bracket_bout(None, None)
assert b["mm"] is None and b["mm0"] is None and b["mm1"] is None, b
print("  no ranger:          keys present, all None")
