...
2_perception: the stage-1 instrument, plus DISTANCE when the beam has a target)
...
DISTANCE, NEW IN THIS STAGE, AND ONLY SOMETIMES. Stage 1 could not measure
translation at all; now a ToF beam looks straight ahead, and when it has a
target — a wall, a person, anything that echoes — every bout is bracketed with
the range at its start and at its end:

    mm0, mm1               the beam's range when the command opened and closed
    mm                     mm0 - mm1: POSITIVE means the gap CLOSED. For a
                           forward bout facing a wall that is distance
                           travelled; for a backward bout expect it negative.

All three are None whenever the beam had nothing to say at either end — no
sensor, no echo (the runtime's honest 0), or a stale reading. A bout with
mm=None is a stage-1 bout: rotation measured, translation only witnessed as
`stir`/`stalled`. That asymmetry is still real — it is why the tuner's `floor`
ramps a SPIN — but it is no longer the whole story: point this body at a wall
and its straight lines become measurable, which is what the wall bench is for.

The ranger is INSTALLED BY THE RUNTIME via set_ranger(), exactly like the
motor writer: the perception pump owns the sensor, this instrument only asks
it "what do you see, and how old is that?" at the two moments that bracket a
bout. The instinct cannot feed it, for the usual reason.
...
class _DriveState:
    ...
    def __init__(self):
        ...
        self.stir_sum = 0.0
        self.samples = 0
        self.mm0 = None          # the beam's range when the bout opened
    ...
    def _close(self, now_s):
        ...
        if commanded and dur >= MIN_BOUT_S:
            ...
            shook = stir >= STIR_G
            # The beam's second bracket. Both ends fresh and echoing, or the
            # bout carries no distance at all — a half-bracket is not a
            # measurement, it is a guess wearing one.
            mm1 = _range_mm() if self.mm0 is not None else None
            self.last_bout = {
                ...
                # the beam's range at open and close, and the difference:
                # POSITIVE = the gap closed. None when either end had no echo.
                "mm0": self.mm0, "mm1": mm1,
                "mm": (self.mm0 - mm1) if mm1 is not None else None,
            }
            self.bouts += 1
            b = self.last_bout
            if b["stalled"]:
            ...
            _tap("bout", motion=b["kind"], speed=b["speed"], for_s=b["for_s"],
                 ...
                 stalled=b["stalled"],
                 mm=(b["mm"] if b["mm"] is not None else -1))
        self.t0 = None
...
# The ranger, installed by main.py the same way: a zero-argument callable
# returning (mm, age_ms), or None when there is no sensor. Bouts sample it at
# open and close; everything else about the beam lives in the pump.
_ranger = [None]

RANGE_FRESH_MS = 300     # a bracket reading older than this is not a bracket

# MEASURED 2026-08-24, first light on the desk (VL53L0X on the HAT SoftI2C —
# which is where the RoverC's chassis Grove ports turned out to be wired):
# against a STABLE target at ~136 mm the beam read 132..140 over 10 s, a
# ±4 mm band. An earlier run read ±12 mm and was believed to be noise until
# the operator pointed out the target had not been still — the instrument
# was measuring the scene, which is its job. Expect the band to widen with
# range (this sensor's spec drifts toward ±3-5% far out), so recipes that
# compare distances of the same order (vfloor's MOVED, step's per-pulse mm)
# still average several settled readings; a bout's mm is trustworthy in
...
def _range_mm():
    """One usable range for bracketing a bout, or None. 0 is the runtime's
    honest 'no echo' and unusable here — a bout bracketed against nothing
    would score the wall's absence as travel."""
    if _ranger[0] is None:
        return None
    try:
        r = _ranger[0]()
    except Exception:
    ...
    if r is None:
        return None
    mm, age_ms = r
    if mm is None or mm <= 0 or age_ms > RANGE_FRESH_MS:
        return None
    return mm
...
class _Drive:
    ...
    def last(self):
        """The most recent scored command, or None. Consumed on read.
            ...
            {"kind": "forward", "speed": 40, "for_s": 1.2, "turned": 3.0,
             "dps": 2.5, "stir": 0.08, "stalled": False,
        ...
        `mm0`/`mm1` are the ToF beam's range when the command opened and
        closed; `mm` is mm0 - mm1, POSITIVE when the gap CLOSED. All three are
        None unless the beam had a fresh echo at BOTH ends — so facing a wall
        they are travel, facing nothing they are honestly absent, and facing
        something that moved they are the two of you combined, which only you
        can untangle."""
        b = _drive.last_bout
