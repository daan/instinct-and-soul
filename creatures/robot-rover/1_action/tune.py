"""
tune.py — interactive motor tuner for robot-rover (M5StickS3 + M5Stack RoverC).

Run:
    flash creatures/robot-rover/1_action --wifi <profile>
    tune  creatures/robot-rover/1_action
...
Commands:
  ...
  scan                               — is the rover there and POWERED? Wants
                                       0x38 on the HAT bus. Run it first every
...
# the order to run them in
    ...
    0. scan       0x38 answers. If it does not, stop and fix that first.
    1. imulog     nothing moves. Is the IMU alive, and does yaw go POSITIVE
                  when you twist the rover clockwise seen from above? Every
                  `turned` in every recipe below rests on that.
    2. motors     rover on a book. Write down, per index, which corner turned
                  and whether it turned forward. Look at the rollers while you
                  are there: they must form an X seen from above, not a
                  diamond. Put MOTOR_CORNER and MOTOR_SIGN into lib/drive.py
                  and reflash — until then every verb is a no-op and says so.
    3. slide      on the floor. Moves sideways, nose still, `turned` near
                  zero. This is the check robot-bug never needed and the one
                  a wrong corner map cannot survive.
    4. floor      on the floor, and again on carpet. SPEED_FLOOR is a fact
                  about the surface as much as about the body.
    5. straight, spin, box, square — did the calibration hold, and what does
                  a speed buy on this floor?

# what this tuner can and cannot measure

Rotation is MEASURED: the gyro, integrated about the vertical the accelerometer
finds, signed, positive clockwise seen from above. Distance is NOT — no
odometry, no ToF until 2_perception, and double-integrating accelerometer
noise is not measurement. So every straight-line recipe reports the rotation it
did NOT want (veer) plus `stir`, which says only that the body was being shaken
about. On a smooth floor a rover can roll almost silently, so a STALL on a
straight leg deserves a look with your eyes before you believe it. A stall on a
spin is gyro-truth.

That asymmetry is why `floor` ramps a spin.
"""
...
PLACEHOLDER = (
    "joy | scan | motors [speed] [secs] | raw <m0> <m1> <m2> <m3> [secs] | "
    "floor [step] [secs] | spin [speed] [secs] | straight [speed] [secs] | "
    "slide [speed] [secs] | box [speed] [side_s] | square [speed] [side_s] | "
    "drive <fwd> <strafe> <spin> [secs] | imulog | vbat | off"
)
    ...
    async def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        try:
            if cmd in ("joy", "joystick"):
                ...
                self.log_msg("scan: who is on the HAT bus? want 0x38 — and "
                             "check the rover's own power switch", style="cyan")
